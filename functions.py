...
def scan_field(x_pos, y_pos, known, empties, ones, twos, threes, fours, fives, sixes, sevens, flags):
    field = [""] * 16
    for y in y_pos:
        for x in x_pos:
...
def color(x, y):
    ...
    if col == empty:
        ...
        return "1"
    elif pyautogui.pixel(x, y+1) == two:
        return "2"
    elif pyautogui.pixel(x, y+1) == three:
        return "3"
    elif pyautogui.pixel(x+2, y) == four:
        return "4"
    elif pyautogui.pixel(x, y+1) == five:
        return "5"
